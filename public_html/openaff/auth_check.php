...
define('CREDS_FILE_OPENAFF', __DIR__ . '/credentials.json');

function load_credentials() {
    if (file_exists(CREDS_FILE_OPENAFF)) {
        $json = json_decode(file_get_contents(CREDS_FILE_OPENAFF), true);
...
function save_credentials($username, $password) {
    ...
    return file_put_contents(CREDS_FILE_OPENAFF, json_encode($data, JSON_PRETTY_PRINT)) !== false;
}
